...
class Empresa extends ResourceController
{
    public function listar(): ResponseInterface
    {
        ...
        return $this->respond([
            'empresa' => $model->select('id, usuario')->findAll(),
        ]);
    }
    ...
    public function formulario(): ResponseInterface
    {
        ...
        $empresa = $model->getEmpresas($id);
        $configuracoesEmpresa = $model->getConfiguracoesEmpresa($id);

        $model = new ConfiguracoesModel();
        $configuracoes = $model->getConfiguracoes($id);
