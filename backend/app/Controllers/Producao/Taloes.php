...
class Taloes extends BaseController
{
    ...
    public function formularioSaida()
    {
        ...
        $grafico['meta_atingida'] = $taloes->selectSum('quantidade')
            ->like('data_saida', $hoje)
            ->getTaloes()[0]['quantidade'];

        $grafico['meta'] = $metas->where('empresa_id', session()->get('empresa')['id'])
            ->like('data', $hoje)
            ->selectSum('meta')
            ->getMetas()[0]['meta'];

        $grafico['titulo'] = 'Meta Atingida - ' . date('d/m/Y');
