<div class="col-sm-12">
    <div class="card shadow">
        ...
        <div class="d-flex justify-content-sm-between align-items-sm-stretch card-header">
            <form action="/admin/nfe/nova" method="post" class="d-flex">
                <input type="text"
                       name="busca"
                       id="busca"
                       placeholder="Usuário"
        ...
        <div class="card-body">
            <table class="table-sm table-bordered dataTable table-striped mb-3 w-100">
                ...
                <tr role="row">
                    ...
                    <th style="width: 20%" class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Name: activate to sort column descending">Nota</th>
                ...
                <?php foreach ($nfes as $nfe): ?>
                    <tr>
                        <th><?= $nfe['id']; ?></th>
                        <td><?= $nfe['usuario']; ?></td>
                        <td><?= $nfe['funcionario']['nome'] ?? '' ?></td>
                        <td class="text-center">
                            <a href="/admin/usuarios/alterar-permissoes?id=<?= $nfe['id'] ?>" ><i class="fas fa-list"></i></a>
                            <a href="/admin/usuarios/formulario?id=<?= $nfe['id'] ?>" ><i class="fas fa-edit"></i></a>
                            <a href="/admin/usuarios/excluir-usuario?id=<?= $nfe['id'] ?>" onclick="return confirm('Deseja mesmo excluir a usuário <?= $nfe['usuario'] ?>?')"><i class="fas fa-trash"></i></a>
                        </td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
